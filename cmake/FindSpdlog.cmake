...
#
# This module supports requiring a minimum version, e.g. you can do find_package(Spdlog 1.2.3) to require version 1.2.3
# or newer of spdlog.
#
# The following variables are set if spdlog is found. Spdlog_FOUND        - True when the spdlog include directory is
# found. Spdlog_INCLUDE_DIR  - The path to where the spdlog include files are. Spdlog_VERSION      - The version of
# spdlog version
#
# Further the following import target will be created, if Spdlog_FOUND is true spdlog::spdlog_header_only - The
# header-only spdlog library
#
# This module reads hints about search locations from the following environment variables:
#
# Spdlog_ROOT Spdlog_ROOT_DIR
#
...
macro(_spdlog_check_version)
    ...
    string(REGEX MATCH "define[ \t]+SPDLOG_VER_MAJOR[ \t]+([0-9]+)" _spdlog_major_version_match
                 "${_spdlog_version_header}"
    )
    set(SPDLOG_MAJOR_VERSION "${CMAKE_MATCH_1}")

    string(REGEX MATCH "define[ \t]+SPDLOG_VER_MINOR[ \t]+([0-9]+)" _spdlog_minor_version_match
                 "${_spdlog_version_header}"
    )
    set(SPDLOG_MINOR_VERSION "${CMAKE_MATCH_1}")

    string(REGEX MATCH "define[ \t]+SPDLOG_VER_PATCH[ \t]+([0-9]+)" _spdlog_patch_version_match
                 "${_spdlog_version_header}"
    )
    ...
    if(NOT Spdlog_VERSION_OK)
        ...
        message(
            STATUS
                "spdlog version ${Spdlog_VERSION} found in ${Spdlog_INCLUDE_DIR}, but at least version ${Spdlog_FIND_VERSION} is required"
        )
...
if(EXISTS "${Spdlog_INCLUDE_DIR}")
    ...
    # search first if an SpdlogConfig.cmake is available in the system, if successful this would set Spdlog_INCLUDE_DIR
...
if(Spdlog_FOUND AND NOT TARGET spdlog::spdlog_header_only)
    ...
    set_target_properties(
        spdlog::spdlog_header_only PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${Spdlog_INCLUDE_DIR}/include"
    )

...
namespace elsa::cublas
{
    namespace detail 
    {
        ...
        template <class ItType>
        static constexpr bool is_fcomplex = std::is_same<iret_type<ItType>, thrust::complex<float>>::value;
        template <class ItType>
        static constexpr bool is_dcomplex = std::is_same<iret_type<ItType>, thrust::complex<double>>::value;
        template <class ItType>
        static constexpr bool is_cublas_type = (is_float<ItType> || is_double<ItType> || is_fcomplex<ItType> || is_dcomplex<ItType>) && thrust::is_contiguous_iterator<ItType>::value;
    ...
    bool inplaceMulScalar(ItType begin, ItType end, const Scalar& scalar) 
    {
        ...
        if constexpr (detail::is_float<ItType>)
            out = cublasSscal(detail::_cublasState.handle, static_cast<int>(count), (const float*)&scalar, (float*)&(*begin), 1);
        else if constexpr (detail::is_double<ItType>)
            out = cublasDscal(detail::_cublasState.handle, static_cast<int>(count), (const double*)&scalar, (double*)&(*begin), 1);
        else if constexpr (detail::is_fcomplex<ItType>)
            out = cublasCscal(detail::_cublasState.handle, static_cast<int>(count), (const cuComplex*)&scalar, (cuComplex*)&(*begin), 1);
        else if constexpr (detail::is_dcomplex<ItType>)
            out = cublasZscal(detail::_cublasState.handle, static_cast<int>(count), (const cuDoubleComplex*)&scalar, (cuDoubleComplex*)&(*begin), 1);

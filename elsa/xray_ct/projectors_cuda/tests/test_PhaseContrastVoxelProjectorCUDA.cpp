...
TEST_SUITE_BEGIN("phase contrast blob projector cuda");
    ...
    for (index_t size = 128; size <= 256; size *= 2) {
        ...
        GIVEN("a volume " + std::to_string(size) + "^2, at 45 degree")
        {
            ...
            WHEN("Setting only the voxels directly on the middle ray to 1")
            {
                ...
                {
                    data_t total_weight = (size) *op.weight(middlePixel + distance_from_center(0));
            ...
            WHEN("Setting only the voxels two steps below the ray")
            {
                ...
                THEN("The detector value is equal to the weight at the scaled distances")
                {
                    ...
                    CHECK_EQ(Ax[0], Approx(total_weight));
                    total_weight = (size - 2) * op.weight(-middlePixel + distance_from_center(-2));
                    CHECK_EQ(Ax[2], Approx(total_weight));
                }
            }
            ...
            WHEN("Setting the 9 middle rays to 1")
            {
                ...
                THEN("The detector value is equal to size * the weight of distance 0")
                {
                    // relative positions of the rays  1 - posB, 1 - posA, 1, 1 + posA, (1 + posB)
                    data_t total_weight =
                        (size - 4) * op.weight(middlePixel + distance_from_center(-4))
                        + (size - 3) * op.weight(middlePixel + distance_from_center(-3))
                        + (size - 2) * op.weight(middlePixel + distance_from_center(-2))
                        + (size - 1) * op.weight(middlePixel + distance_from_center(-1))
                        + size * op.weight(middlePixel + distance_from_center(0))
                        + (size - 1) * op.weight(middlePixel + distance_from_center(1))
            ...
            WHEN("Setting all the voxels along a slice to a distribution")
            {
                ...
                THEN("The detector value is equal to size * the weight of distance 0")
                {
                    // relative positions of the rays  1 - posB, 1 - posA, 1, 1 + posA, (1 + posB)
                    data_t total_weight =
                        .6 * (size - 4) * op.weight(middlePixel + distance_from_center(-4))
                        + .7 * (size - 3) * op.weight(middlePixel + distance_from_center(-3))
                        + .8 * (size - 2) * op.weight(middlePixel + distance_from_center(-2))
                        + .9 * (size - 1) * op.weight(middlePixel + distance_from_center(-1))
                        + size * op.weight(middlePixel + distance_from_center(0))
                        + .9 * (size - 1) * op.weight(middlePixel + distance_from_center(1))
                        + .8 * (size - 2) * op.weight(middlePixel + distance_from_center(2))
                        + .7 * (size - 3) * op.weight(middlePixel + distance_from_center(3))
            ...
            WHEN("Using a smoothBlob")
            {
                ...
                THEN("The detector value is equal to size * the weight of distance 0")
                {
                    ...
                    for (index_t i = 0; i < size; i++) {
                        total_weight += x(i, i) * op.weight(middlePixel + distance_from_center(0));
                    }
                    for (index_t i = 0; i < size - 1; i++) {
                        total_weight +=
                            x(i + 1, i) * op.weight(middlePixel + distance_from_center(1));
                    }
                    for (index_t i = 0; i < size - 1; i++) {
                        total_weight +=
                            x(i, i + 1) * op.weight(middlePixel + distance_from_center(-1));
                    }
                    for (index_t i = 0; i < size - 2; i++) {
                        total_weight +=
                            x(i + 2, i) * op.weight(middlePixel + distance_from_center(2));
                    }
                    for (index_t i = 0; i < size - 2; i++) {
                        total_weight +=
                            x(i, i + 2) * op.weight(middlePixel + distance_from_center(-2));
                    }
                    for (index_t i = 0; i < size - 3; i++) {
                        total_weight +=
                            x(i + 3, i) * op.weight(middlePixel + distance_from_center(3));
                    }
                    for (index_t i = 0; i < size - 3; i++) {
                        total_weight +=
                            x(i, i + 3) * op.weight(middlePixel + distance_from_center(-3));
                    }
                    for (index_t i = 0; i < size - 4; i++) {
                        total_weight +=
                            x(i + 4, i) * op.weight(middlePixel + distance_from_center(4));
                    }
                    for (index_t i = 0; i < size - 4; i++) {
                        total_weight +=
                            x(i, i + 4) * op.weight(middlePixel + distance_from_center(-4));
                    }
                    CHECK_EQ(Ax[0], Approx(total_weight).epsilon(0.001));
                    CHECK_EQ(Ax[2], Approx(-total_weight).epsilon(0.001));
                }
    ...
    for (index_t size = 8; size <= 256; size *= 2) {
        ...
        GIVEN("a volume " + std::to_string(size) + "^2, at 45 degree")
        {
            ...
            WHEN("Setting only the voxels directly on the ray to 1")
            {
                ...
                const auto weight = op.weight(distance_from_center(0));
                CAPTURE(weight);

 ...
 *
 * We always normalize the SphericalHarmoncis with respect to the measure given as sphericalFunctionDesc->getWeights().sum()!
 * Such that \f$ \Omega(S_2) \f$
 *
 * \author Max Endrass (endrass@cs.tum.edu), most boilerplate code courtesy of Matthias Wieczorek
 * \author Matthias Wieczorek (wieczore@cs.tum.edu), logic merge with XTTSphericalHarmonicsDescriptor and fixes for symmetry cases
 * \author Nikola Dinev (nikola.dinev@tum.de), port to elsa
 *
 * \tparam data_t real type
 */
...
public:
     ...
     * \param[in] rangeDescriptor Descriptor for result range (max degree of SH polynomials etc.)
     */
    ...
    void applyAdjointImpl(const DataContainer<data_t>& x, DataContainer<data_t>& Atx) const override;
...
private:
    ...
    MatrixXd_t getSphericalHarmonicsBasis(const SphericalFunctionDescriptor<data_t>& sphericalFunctionDesc,
                                                                      const SphericalHarmonicsDescriptor& sphericalHarmonicsDesc);

    VolumeDescriptor getBasisDescriptor(const SphericalFunctionDescriptor<data_t>& sphericalFunctionDesc,
                                        const SphericalHarmonicsDescriptor& sphericalHarmonicsDesc);
    VolumeDescriptor getBasisTransposeDescriptor(const SphericalFunctionDescriptor<data_t>& sphericalFunctionDesc,
                                        const SphericalHarmonicsDescriptor& sphericalHarmonicsDesc);

...
namespace elsa
{
    ...
    AXDTStatRecon<data_t>::AXDTStatRecon(const DataContainer<data_t>& ffa,
        ...
        : Functional<data_t>(generate_descriptors(absorp_op.getDomainDescriptor(), axdt_op.getDomainDescriptor())),
          ffa_(ffa), ffb_(ffb), a_tilde_(a), b_tilde_(b), absorp_op_(absorp_op.clone()), axdt_op_(axdt_op.clone()),
          N_(static_cast<data_t>(N)), recon_type_(recon_type)
    {
        alpha_ = *ffb_ / *ffa_;
    ...
    AXDTStatRecon<data_t>::AXDTStatRecon(const DataContainer<data_t>& axdt_proj,
        ...
        : Functional<data_t>(generate_descriptors(*generate_placeholder_descriptor(), axdt_op.getDomainDescriptor())),
          absorp_op_(nullptr), axdt_op_(axdt_op.clone()),
          N_(static_cast<data_t>(0)), recon_type_(recon_type)
    {
        d_tilde_ = exp(-axdt_proj);
        if (recon_type == Gaussian_approximate_racian ||
            recon_type == Racian_direct)
        {
            throw std::invalid_argument("flat-field data required for requested reconstruction type");
        }
    }
    ...
    data_t AXDTStatRecon<data_t>::evaluateImpl(const DataContainer<data_t>& Rx)
    {
        ...
        switch (recon_type_) {
            ...
            case Racian_direct:
            {
                ...
                term_2 *=
                    (static_cast<data_t>(2) * square(a))
                    + (static_cast<data_t>(2) * square(*a_tilde_))
                    + square(*b_tilde_)
                                                ...
                                                DataContainer<data_t>& out)
    {
        ...
        switch (recon_type_) {
            case Gaussian_log_d:
                grad_mu = 0;
                grad_eta = axdt_op_->applyAdjoint(static_cast<data_t>(2.0) * (log_d - log(*d_tilde_)));
                break;
...
//            data_t max_eta = 0;
//            data_t min_eta = 1000;
//            for (int i = 0; i < grad_mu.getDataDescriptor().getNumberOfCoefficients(); ++i) {
//                max_mu = std::max<data_t>(max_mu, grad_mu[i]);
//                min_mu = std::min<data_t>(min_mu, grad_mu[i]);
//            }
//            for (int i = 0; i < grad_eta.getDataDescriptor().getNumberOfCoefficients(); ++i) {
...
//        }
    ...
    LinearOperator<data_t>
        AXDTStatRecon<data_t>::getHessianImpl(const DataContainer<data_t>& Rx)
    {
        ...
        switch(recon_type_)
        {
            case Gaussian_log_d:
            {
                auto hessian_absorp_0 = std::make_unique<ZeroOperator<data_t>>(*generate_placeholder_descriptor(),
                                                                               *generate_placeholder_descriptor());
                auto hessian_absorp_1 = std::make_unique<ZeroOperator<data_t>>(axdt_op_->getDomainDescriptor(),
                                                                               *generate_placeholder_descriptor());
                ops.clear();
                ...
                auto hessian_axdt_0 = std::make_unique<ZeroOperator<data_t>>(*generate_placeholder_descriptor(),
                                                                             axdt_op_->getDomainDescriptor());
            ...
            case Gaussian_d:
            {
                ...
                auto hessian_axdt_1 = std::make_unique<LinearOperator<data_t>>(
                    ...
                    * Scaling<data_t>(axdt_op_->getRangeDescriptor(),
                                      static_cast<data_t>(2.0) * d * (static_cast<data_t>(2.0) * d - *d_tilde_)
                                      )
                    * *axdt_op_);
                ops.clear();
                ops.emplace_back(hessian_axdt_0->clone());
            ...
            case Gaussian_approximate_racian:
            {
                ...
                auto hessian_absorp_0 = std::make_unique<LinearOperator<data_t>>(
                    adjoint(*absorp_op_) *
                    Scaling<data_t>(absorp_op_->getRangeDescriptor(), H_1_1) * *absorp_op_
                    );
                auto hessian_absorp_1 = std::make_unique<LinearOperator<data_t>>(
                    adjoint(*absorp_op_) *
                    Scaling<data_t>(axdt_op_->getRangeDescriptor(), H_1_2) * *axdt_op_
                    );
                ops.clear();
                ops.emplace_back(hessian_absorp_0->clone());
                ops.emplace_back(hessian_absorp_1->clone());
                ...
                auto hessian_axdt_0 = std::make_unique<LinearOperator<data_t>>(
                    adjoint(*axdt_op_) *
                    Scaling<data_t>(absorp_op_->getRangeDescriptor(), H_1_2) * *absorp_op_
                    );
                auto hessian_axdt_1 = std::make_unique<LinearOperator<data_t>>(
                    adjoint(*axdt_op_) *
                    Scaling<data_t>(axdt_op_->getRangeDescriptor(), H_2_2) * *axdt_op_
                    );
                ops.clear();
                ops.emplace_back(hessian_axdt_0->clone());
                ops.emplace_back(hessian_axdt_1->clone());
    ...
    AXDTStatRecon<data_t>* AXDTStatRecon<data_t>::cloneImpl() const
    {
        switch (recon_type_)
        {
            ...
            case Gaussian_approximate_racian:
            case Racian_direct:
                return new AXDTStatRecon(*ffa_, *ffb_, *a_tilde_, *b_tilde_, *absorp_op_,
                                         *axdt_op_, static_cast<index_t>(N_), recon_type_);
        }
    }
    ...
    bool AXDTStatRecon<data_t>::isEqual(const Functional<data_t>& other) const
    {
        ...
        switch (recon_type_)
        {
            ...
            case Gaussian_approximate_racian:
            case Racian_direct:
                if (otherAXDTStatRecon->ffa_ != ffa_ ||
                    otherAXDTStatRecon->ffb_ != ffb_ ||

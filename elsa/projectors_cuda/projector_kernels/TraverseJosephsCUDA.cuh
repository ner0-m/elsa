...
namespace elsa
{
    ...
    struct TraverseJosephsCUDA {
        ...
        struct BoundingBox {
            ...
            __device__ __forceinline__ real_t& operator[](const uint32_t idx) { return max[idx]; }
        };
         ...
         */
        static void
            traverseForward(const dim3 blocks, const int threads, cudaTextureObject_t volume,
                            ...
                            const BoundingBox& boxMax, const cudaStream_t stream = 0);
         ...
         * \param[in] stream handle to stream in which the kernel should be placed
         *
         * The variables blocks and threads should be picked based on the volume dimensions. To
         * process all voxels set blocks to (volSizeX, volSizeY, volSizeZ / threads), if volSizeZ is
         * not a multiple of threads a second kernel call must be made to process the remaining
         * voxels with blocks = (volSizeX, volSizeY, 1) and threads = volSizeZ % threadsFirstCall.
         * Volume pointers should be adjusted accordingly to point to the start of
         * volume[0][0][volSizeZ - volSizeZ % threadsFirstCall].
         *
         * threads should ideally be a multiple of the warp size (32 for all current GPUs).
         *
         * \warning Interpolation mode for texture should be set to cudaFilterModePoint when working
        ...
        static void traverseAdjointFast(const dim3 blocks, const int threads,
                                        ...
                                        const uint32_t zOffset = 0, const cudaStream_t stream = 0);
    };
} // namespace elsa
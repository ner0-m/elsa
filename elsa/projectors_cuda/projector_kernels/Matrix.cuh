 ...
 */
template <typename real_t, uint32_t dim, typename Array>
__device__ __forceinline__ void gehomv(const int8_t* const __restrict__ matrix,
                                       const real_t* const __restrict__ vector, Array result,
                                       const uint32_t matrixPitch)
{
    // initialize result vector
...
/// determine reverse norm of vector of length 2 or 3 using device inbuilt functions
template <typename real_t, uint32_t dim, typename Array>
__device__ __forceinline__ real_t norm(Array vector)
{
    if (dim == 3)
        return norm3d(vector[0], vector[1], vector[2]);
    else if (dim == 2)
...
template <typename real_t, uint32_t dim>
__device__ __forceinline__ void homogenousNormalize(real_t* vector)
{
    real_t w = 1 / vector[dim - 1];

...
namespace elsa
{
              ...
              _boundingBox(_domainDescriptor->getNumberOfCoefficientsPerDimension())
    {
        ...
        if(cudaMallocPitch(&_rayOrigins.ptr, &_rayOrigins.pitch,dim*sizeof(real_t),geometryList.size())!=cudaSuccess)
            throw std::bad_alloc();
        ...
        for (index_t i=0;i<geometryList.size();i++) {
            ...
            if (cudaMemcpy2DAsync(slice, projPitch, P.data(), dim*sizeof(real_t), dim*sizeof(real_t), dim, cudaMemcpyHostToDevice) != cudaSuccess)
                throw std::logic_error("SiddonsMethodCUDA: Could not transfer inverse projection matrices to GPU.");
    ...
    template <typename data_t>
    void SiddonsMethodCUDA<data_t>::_apply(const DataContainer<data_t>& x, DataContainer<data_t>& Ax) const
    {
    ...
    template <typename data_t>
    void SiddonsMethodCUDA<data_t>::_applyAdjoint(const DataContainer<data_t>& y, DataContainer<data_t>& Aty) const
    {
    ...
    void SiddonsMethodCUDA<data_t>::traverseVolume(void* volumePtr, void* sinoPtr) const
    {
        ...
        if (numDim==3) {
            ...
            if(adjoint)
                copy3DDataContainerGPU<cudaMemcpyDeviceToHost,false>(volumePtr,dvolumePtr,volExt);
            else 

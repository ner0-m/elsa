...
namespace elsa
{
     ...
     * This signal can be n-dimensional, and will be stored in memory in a linearized fashion.
     */
     ...
     public:
          ...
          *
          * \param[in] numberOfCoefficientsPerDimension vector containing the number of coefficients per dimension,
          *            (dimension is set implicitly from the size of the vector)
          *
          ...
          */
         explicit DataDescriptor(IndexVector_t numberOfCoefficientsPerDimension, RealVector_t spacingPerDimension);
          ...
          *
          * The local coordinates are integers, running from 0 to _numberOfCoefficientsPerDimension[i]-1 for every
          * dimension i = 0,...,_numberOfDimensions. Linearization is assumed to be done in order of the dimensions.
          */

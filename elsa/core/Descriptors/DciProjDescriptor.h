...
namespace elsa
{
    ...
    public:
        ...
        DciProjDescriptor(const DataDescriptor& desc, const std::vector<Geometry>& geom,
                          const DirVec& sensDir = DirVec(1, 0, 0), bool parallelBeamFlag = true);
    ...
    private:
        ...
        const DirVec
            _sensDir; ///< sensitivity direction (inplane orthogonal vector to the gratings)
        bool _parallelBeamFlag; ///< indicating if the geometry is assumed to represent an

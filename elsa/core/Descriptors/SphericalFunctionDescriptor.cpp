...
SphericalFunctionDescriptor<data_t>::SphericalFunctionDescriptor(
    const DirVecList &dirs,
      ...
      _weights(weights)
{
    if (dirs.size() != static_cast<size_t>(weights.size()))
        throw std::invalid_argument("SphericalFunctionDescriptor: Sizes of directions list and weights do not match.");

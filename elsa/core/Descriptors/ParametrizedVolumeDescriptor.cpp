...
namespace elsa
{
          ...
          _basisDescriptor(basisDesciptor.clone())
    {
        if (basisDesciptor.getNumberOfDimensions() != 1)
            throw std::invalid_argument(
                "ParametrizedVolumeDescriptor: Basis should be exactly one-dimensional.");
    }
    ...
    //        volumeDimensions[volumeDimensions.size()-1] =
    //        basisDesciptor.getNumberOfCoefficients();; return volumeDimensions;
    //    }
    //
    ...
    {
        return new ParametrizedVolumeDescriptor(IdenticalBlocksDescriptor::getDescriptorOfBlock(0),
                                                *_basisDescriptor);
    }

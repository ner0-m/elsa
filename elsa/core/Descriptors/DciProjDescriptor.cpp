...
namespace elsa {
    ...
    DciProjDescriptor::DciProjDescriptor(const DataDescriptor& desc, const std::vector<Geometry>& geom, const DirVec& sensDir, bool parallelBeamFlag)
        : IdenticalBlocksDescriptor(geom.size(), desc),
          _geom(geom), _sensDir(sensDir.normalized()), _parallelBeamFlag(parallelBeamFlag)
    {}
    ...
    DciProjDescriptor* DciProjDescriptor::cloneImpl() const {
        return new DciProjDescriptor(IdenticalBlocksDescriptor::getDescriptorOfBlock(0), _geom, _sensDir, _parallelBeamFlag);
    }

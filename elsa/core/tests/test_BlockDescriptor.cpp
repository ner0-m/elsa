...
SCENARIO("Constructing BlockDescriptors") {
    GIVEN("a 1D descriptor with blocks") {
        ...
        WHEN("creating 5 blocks") {
            ...
            THEN("the new BlockDescriptor does index calculations correctly") {
                IndexVector_t coordinate(2); coordinate << dd.getNumberOfCoefficients()-1, blocks-1;
    ...
    GIVEN("a 2D descriptor with blocks") {
        IndexVector_t sizeVector(2); sizeVector << 11, 12;
        ...
        WHEN("creating 10 blocks") {
            ...
            THEN("the new BlockDescriptor does index calculations correctly") {
                IndexVector_t coordinate(3); coordinate << sizeVector(0)-3, sizeVector(1)-7, blocks-2;
                index_t index = coordinate(0) + coordinate(1) * sizeVector(0) + coordinate(2) * sizeVector(0)*sizeVector(1);
    ...
    GIVEN("a 3D descriptor with blocks") {
        IndexVector_t sizeVector(3); sizeVector << 101, 42, 57;
        DataDescriptor dd(sizeVector);
        ...
        WHEN("creating 25 blocks") {
            ...
            THEN("the new BlockDescriptor does index calculations correctly") {
                IndexVector_t coordinate(4); coordinate << sizeVector(0)-33, sizeVector(1)-11, sizeVector(2)-17, blocks-19;
                index_t index = coordinate(0) + coordinate(1) * sizeVector(0) + coordinate(2) * sizeVector(0)*sizeVector(1)
...
SCENARIO("Cloning BlockDescriptors") {
    GIVEN("a 1D BlockDescriptor") {
        IndexVector_t sizeVector(1); sizeVector << 13;
        DataDescriptor dd(sizeVector);
        index_t blocks = 21;
    ...
    GIVEN("a 2D BlockDescriptor") {
        IndexVector_t sizeVector(2); sizeVector << 43, 112;
        DataDescriptor dd(sizeVector);
        index_t blocks = 77;
    ...
    GIVEN("a 3D BlockDescriptor") {
        IndexVector_t sizeVector(3); sizeVector << 47, 11, 53;
        DataDescriptor dd(sizeVector);
        index_t blocks = 13;

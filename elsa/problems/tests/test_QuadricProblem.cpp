...
TEMPLATE_TEST_CASE("Scenario: Testing QuadricProblem", "", QuadricProblem<float>, QuadricProblem<double>) {
    ...
    GIVEN("a Quadric functional") {
        ...
        WHEN("constructing a QuadricProblem with x0 from it") {
            ...
            THEN("the problem behaves as expected") {
                ...
                REQUIRE(prob.evaluate() == Approx(static_cast<data_t>(0.5*scaleFactor)*x0.squaredL2Norm() - x0.dot(dc)));
    ...
    GIVEN("a non-spd operator") {
        ...
        WHEN("Constructing a QuadricProblem with x0 from it") {
            ...
            THEN("the problem behaves as expected") {
                ...
                REQUIRE(prob.evaluate() == Approx(static_cast<data_t>(0.5*scaleFactor*scaleFactor)*x0.squaredL2Norm() - scaleFactor*x0.dot(dc)));
    ...
    GIVEN("an optimization problem") {
        ...
        WHEN("converting an optimization problem that has only a quadric data term") {
            ...
            THEN("the problem behaves as expected") {
                ...
                REQUIRE(prob.evaluate() == Approx(static_cast<data_t>(0.5*weightFactor)*x0.squaredL2Norm() - x0.dot(dc)));
        ...
        dataTerms.push_back(std::move(std::make_unique<L2NormPow2<data_t>>(LinearResidual<data_t>{scalingOp})));
        dataTerms.push_back(std::move(std::make_unique<L2NormPow2<data_t>>(LinearResidual<data_t>{dc})));
        dataTerms.push_back(std::move(std::make_unique<L2NormPow2<data_t>>(LinearResidual<data_t>{scalingOp,dc})));
        dataTerms.push_back(std::move(std::make_unique<WeightedL2NormPow2<data_t>>(weightingOp)));
        dataTerms.push_back(std::move(std::make_unique<WeightedL2NormPow2<data_t>>(LinearResidual<data_t>{scalingOp}, weightingOp)));
        dataTerms.push_back(std::move(std::make_unique<WeightedL2NormPow2<data_t>>(LinearResidual<data_t>{dc}, weightingOp))); 
        dataTerms.push_back(std::move(std::make_unique<WeightedL2NormPow2<data_t>>(LinearResidual<data_t>{scalingOp,dc}, weightingOp)));

        for (const auto& dataTerm: dataTerms) {
            const LinearResidual<data_t>& res = static_cast<const LinearResidual<data_t>&>(dataTerm->getResidual());
            ...
            std::string desc = probType + (std::string(res.hasOperator() ? " with an operator" : " with no operator")  + ", ") + (res.hasDataVector() ? "a data vector" : "no data vector");
            WHEN("converting a " + desc + ", and no x0 to a quadric problem") {
                // use OptimizationProblem istead of WLSProblem as it allows for more general formulations
                ...
                THEN("the clone works correctly") {
                    ...
                    AND_THEN("the problem behaves as expected") {
                        ...
                        DataContainer<data_t> grad = res.hasDataVector() ? static_cast<data_t>(-1.0)*dc : DataContainer<data_t>{dd};
                        ...
                        if (isWeighted) {
                            if (res.hasOperator()) {
                                REQUIRE(prob.getHessian() == leaf(adjoint(scalingOp)*weightingOp*scalingOp));
                            }
                            else {
                         ...
                         if (isWeighted) {
                            if (res.hasOperator()) {
                                if (res.hasDataVector()) {
                                    REQUIRE(prob.evaluate() == Approx(0.5*scaleFactor*scaleFactor*weightFactor*x0.squaredL2Norm() - scaleFactor*weightFactor*x0.dot(dc)));
                                    REQUIRE(prob.getGradient() == scaleFactor*(weightFactor*(scaleFactor*x0)) - scaleFactor*(weightFactor*dc));
                                }
                                else {
                                    REQUIRE(prob.evaluate() == Approx(0.5*scaleFactor*scaleFactor*weightFactor*x0.squaredL2Norm()));
                                    REQUIRE(prob.getGradient() == scaleFactor*(weightFactor*(scaleFactor*x0)));
                                }
                                REQUIRE(prob.getHessian() == leaf(adjoint(scalingOp)*weightingOp*scalingOp));
                            }
                            else {
                                if (res.hasDataVector()) {
                                    REQUIRE(prob.evaluate() == Approx(0.5*weightFactor*x0.squaredL2Norm() - weightFactor*x0.dot(dc)));
                                    REQUIRE(prob.getGradient() == weightFactor*x0 - weightFactor*dc);
                                }
                                else {
                                    REQUIRE(prob.evaluate() == Approx(0.5*weightFactor*x0.squaredL2Norm()));
                            ...
                            if (res.hasOperator()) {
                                if (res.hasDataVector()) {
                                    REQUIRE(prob.evaluate() == Approx(0.5*scaleFactor*scaleFactor*x0.squaredL2Norm() - scaleFactor*x0.dot(dc)));
                                    REQUIRE(prob.getGradient() == scaleFactor*(scaleFactor*x0) - scaleFactor*dc);
                                }
                                else {
                                    REQUIRE(prob.evaluate() == Approx(0.5*scaleFactor*scaleFactor*x0.squaredL2Norm()));
                                ...
                                if (res.hasDataVector()) {
                                    REQUIRE(prob.evaluate() == Approx(0.5*x0.squaredL2Norm() - x0.dot(dc)));
            ...
            for (const auto& regTerm: dataTerms) {
                const LinearResidual<data_t>& res = static_cast<const LinearResidual<data_t>&>(regTerm->getResidual());
                const auto isWeighted = dynamic_cast<const WeightedL2NormPow2<data_t>*>(regTerm.get());
                std::string regType = isWeighted ? "weighted l2-norm regularizer" : "l2-norm regularizer";

                std::string desc = regType + (std::string(res.hasOperator() ? " with an operator" : " with no operator")  + ", ") + (res.hasDataVector() ? "a data vector" : "no data vector");

                WHEN("converting a Tikhonov problem with a " + desc + ", and no x0 to a quadric problem") {

                    // conversion of data terms already tested, fix to 0.5*||Ax-b||^2 for tikhonov problem tests
                    ...
                    THEN("the clone works correctly") {
                        ...
                        AND_THEN("the problem behaves as expected") {
                            ...
                            if (isWeighted) {
                                ...
                                if (res.hasOperator()) {
                                    REQUIRE(prob.getHessian() == leaf(adjoint(dataScalingOp)*dataScalingOp + adjoint(scalingOp)*lambdaW*scalingOp));
                                }
                                else {
                                    REQUIRE(prob.getHessian() == leaf(adjoint(dataScalingOp)*dataScalingOp + lambdaW));
                                }
                            }
                            else {
                                ...
                                if (res.hasOperator()) {
                                    REQUIRE(prob.getHessian() == leaf(adjoint(dataScalingOp)*dataScalingOp + lambdaOp*adjoint(scalingOp)*scalingOp));
                                }
                                else {
                                    REQUIRE(prob.getHessian() == leaf(adjoint(dataScalingOp)*dataScalingOp + lambdaOp));
                                }
                            }                        
                        }
                    }   
                ...
                WHEN("converting a Tikhonov problem with a " + desc + ", and x0 to a quadric problem") {
                    randomData.setRandom();
                    DataContainer<data_t> x0{dd,randomData};
                    ...
                    THEN("the clone works correctly") {
                        ...
                        AND_THEN("the problem behaves as expected") {
                            ...
                            if (isWeighted) {
                                ...
                                if (res.hasOperator()) {
                                    Ax += scaleFactor*(regWeight*weightFactor*(scaleFactor*x0));
                                    if (res.hasDataVector()) {   

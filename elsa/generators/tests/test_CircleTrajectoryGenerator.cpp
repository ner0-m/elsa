...
SCENARIO("Create a Circular Trajectory") {
    ...
    GIVEN("A 2D descriptor and 256 angles") {
        ...
        WHEN("We create a half circular trajectory for this scenario") {
            ...
            auto [geomList, sdesc] = CircleTrajectoryGenerator::createTrajectory(numberOfAngles, desc, halfCircular, diffCenterSource, diffCenterDetector);

            THEN("Every geomList in our list has the same camera center and the same projection matrix") {
                ...
                for (int i = 0; i < numberOfAngles; ++i) {
                    ...
                    REQUIRE( (tmpGeom.getCameraCenter() - geomList[i].getCameraCenter()).norm() == Approx(0) );
                    REQUIRE( (tmpGeom.getProjectionMatrix() - geomList[i].getProjectionMatrix()).norm() == Approx(0).margin(0.0000001) );
                    REQUIRE( (tmpGeom.getInverseProjectionMatrix() - geomList[i].getInverseProjectionMatrix()).norm() == Approx(0).margin(0.0000001) );
                }
            }
        }

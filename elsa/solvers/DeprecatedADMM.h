...
namespace elsa
{
    ...
    class DeprecatedADMM : public Solver<data_t>
    {
    public:
        /// Scalar alias
        using Scalar = typename Solver<data_t>::Scalar;

        DeprecatedADMM(const SplittingProblem<data_t>& splittingProblem)
            : Solver<data_t>(splittingProblem)
        {
            static_assert(std::is_base_of<Solver<data_t>, XSolver<data_t>>::value,
                          "DeprecatedADMM: XSolver must extend Solver");

            static_assert(std::is_base_of<ProximityOperator<data_t>, ZSolver<data_t>>::value,
                          "DeprecatedADMM: ZSolver must extend ProximityOperator");
        }
        ...
        /// default destructor
        ~DeprecatedADMM() override = default;
        ...
        auto solveImpl(index_t iterations) -> DataContainer<data_t>& override
        {
            ...
            if (g.size() != 1) {
                throw InvalidArgumentError(
                    "DeprecatedADMM::solveImpl: supported number of regularization terms is 1");
            }
            ...
            if (!is<L0PseudoNorm<data_t>>(regularizationTerm)
                ...
                throw InvalidArgumentError(
                    "DeprecatedADMM::solveImpl: supported regularization terms are "
    ...
    protected:
        ...
        auto cloneImpl() const -> DeprecatedADMM<XSolver, ZSolver, data_t>* override
        {
            return new DeprecatedADMM<XSolver, ZSolver, data_t>(

/**
 * @file test_BA_GMRES.cpp
 *
 * @brief Tests for the BA_GMRES class
 *
...
TYPE_TO_STRING(BA_GMRES<float>);
TYPE_TO_STRING(BA_GMRES<double>);
...
TEST_CASE("Clone and Comparison Operation")
{
    ...
    GIVEN("a Phantom reconstruction problem")
    {
        ...
        // projector backprojector sinogram epsilon
        real_t epsilon = std::numeric_limits<real_t>::epsilon();

        WHEN("setting up a BA_GMRES solver")
        {
            BA_GMRES solver{projector, sinogram, epsilon};
...
TEST_CASE("Comparison of restarted and non-restarted GMRES")
{
    ...
    GIVEN("a Phantom reconstruction problem")
    {
        ...
        WHEN("setting up BA_GMRES solvers")
        {
            BA_GMRES solver{projector, sinogram, epsilon};
...
TEST_CASE("Non-Square, Non-Symmetric Unmatched BA Matrix Reconstruction")
{
    Logger::setLevel(Logger::LogLevel::OFF);
    ...
    GIVEN("A 2x3 Matrix Operator A, 3x2 Matrix Operator B, a 3x1 Vector Datacontainer b and no x0")
    {
        ...
        WHEN("Setting up a simple unmatched BA_GMRES solver")
        {
            BA_GMRES solver{A, B, b, epsilon};

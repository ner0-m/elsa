...
namespace elsa
{
    ...
    CGNL<data_t>::CGNL(const Problem<data_t>& prob, data_t eps_CG,
                       data_t eps_NR, index_t iterations_NR, index_t restart)
        : prob_(prob.clone()), eps_CG_(eps_CG), eps_NR_(eps_NR), iterations_NR_(iterations_NR),
          restart_(restart)
    {
    }
                                              ...
                                              std::optional<DataContainer<data_t>> x0)
    {
        ...
        auto x = DataContainer<data_t>(prob_->getDataTerm().getDomainDescriptor());
        auto r = DataContainer<data_t>(prob_->getDataTerm().getDomainDescriptor());
        auto d = DataContainer<data_t>(prob_->getDataTerm().getDomainDescriptor());
        ...
        for (index_t iter_CG = 0; iter_CG < iterations_CG; ++iter_CG) {
            ...
            for (index_t iter_NR = 0; iter_NR < iterations_NR_; ++iter_NR) {
                data_t alpha_numerator = static_cast<data_t>(-1.0) * prob_->getGradient(x).dot(d);

                auto hessian = prob_->getHessian(x);
...
//            Logger::get("CGNL")->info("delta_new {}", delta_new);
    ...
    {
        return new CGNL(*prob_, eps_CG_, eps_NR_, iterations_NR_, restart_);
    }
    ...
    bool CGNL<data_t>::isEqual(const Solver<data_t>& other) const
    {
        ...
        return otherCGNL && *(otherCGNL->prob_) == *prob_

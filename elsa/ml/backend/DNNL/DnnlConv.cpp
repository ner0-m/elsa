...
namespace elsa
{
    ...
    {
        // Set descriptors and memory for weights
        ...
        _weightsMemory = dnnl::memory(
            {{_weightsDimensions}, _typeTag, dnnl::memory::format_tag::oihw}, *_engine);
    ...
    template <typename data_t>
    void DnnlConv<data_t>::setWeights(const DataContainer<data_t>& weights)
    {
        this->writeToDnnlMemory(weights, _weightsMemory);
    }

    template <typename data_t>
    void DnnlConv<data_t>::setBias(const DataContainer<data_t>& bias)
    {
        this->writeToDnnlMemory(bias, _biasMemory);
    }

    template <typename data_t>
    void DnnlConv<data_t>::setInitializer(Initializer initializer)
    {

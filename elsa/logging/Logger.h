...
namespace elsa
{
     ...
     *
     * This class provides logging for the elsa library. It is a thin wrapper around the spdlog third party library,
     * using the single-threaded versions of the sinks.
     *
     * Logger::get("name") requests a logger (from spdlog) corresponding to "name", which is used just as you would use
     * spdlog directly, i.e. you can use the info(), debug(), warn() etc. routines. For more  details on spdlog, please
     * visit https://github.com/gabime/spdlog.
     *
     * By default, Logger enables console logging. If requested, file logging can enabled in addition via addFileSink.
     * Log levels are set via setLevel (again, they correspond to the spdlog log levels).
     */
     class Logger {
     public:

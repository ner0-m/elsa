...
namespace detail
{
    ...
    void add_linear_operator(py::module& m, const char* name)
    {
     ...
     void dlpack_capsule_destructor(PyObject *self) {
     ...
     py::object build_data_container(DLManagedTensor *tensor) {
          ...
          switch (device.device_type) {
               ...
               case kDLCUDAManaged:
                    /* copy into container */
                    /* TODO: potentially create view for kDLCUDAManaged
                       (then also rename capsule and have view not deleted by other library) */
                    /* TODO: use different memcpy, if its cuda memory */
                    if(strided_copy) {
                         /* TODO */
                         PyErr_SetString(PyExc_RuntimeError, "data with non row major densely packed layout not yet supported.");
                         return py::none();
                    }
                    std::memcpy(dc->storage().data().get(), tensor->dl_tensor.data + tensor->dl_tensor.byte_offset, shape.prod() * sizeof(data_t));
     ...
     void add_data_container_dlpack(py::module& m, py::class_<elsa::DataContainer<data_t>> dc) {
          dc.def("__dlpack__", [](elsa::DataContainer<data_t>& self, py::object stream) -> py::object {
               ...
               /* TODO: check if elsa is built with CUDA */
#if 0
               ...
               /* pybind can translate potential exceptions generated by this */
               ...
               uintptr_t data_ptr = reinterpret_cast<uintptr_t>(self.storage().data().get());
               ...
               managed->deleter = tensor_destructor;

               return py::capsule(managed, "dltensor", dlpack_capsule_destructor);
          });

          m.def("from_dlpack", [](py::object obj) -> py::object {
    ...
    void add_data_container(py::module& m, const char* name)
    {
        ...
        add_data_container_dlpack(m, dc);
    }
} // namespace detail

...
void write(elsa::DataContainer<double>& phantom, std::string prae, int dimension)
{
    ...
    for (int i = 0; i < max; i++) {
        ...
        std::string fullPath{prae};
        fullPath.append("/");
        fullPath.append(name);
        io::write(phantom.slice(sli), fullPath);
        Logger::get("Ellipsoid")->info("Export to {} slice {}", fullPath, sli);
    }
}
...
void example3d()
{
    ...
    auto phantom = phantoms::forbildHead<double>(size);
    write(phantom, std::string{"head"}, dimension);
    phantom = 0;

    phantom = phantoms::forbildThorax<double>(size);
    write(phantom, std::string{"thorax"}, dimension);
    phantom = 0;

    phantom = phantoms::forbildAbdomen<double>(size);

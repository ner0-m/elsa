...
The full dataset is reconstructed using the FISTA/APGD solver. The problem
is the LASSO problem, i.e. L1 regularization. This is nothing fancy, but it works
well. Checkout the ADMM example to see a more advanced example for the reconstruction.

This example is mostly to show how to work with a real world dataset. See
the 'load_htc2022data' function.
"""
import argparse
...
def reconstruction(A, b, niters=70, reg=7.0):
    """Reconstruction with APGDA/FISTA"""
    prox = elsa.ProximalL1(reg)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Reconstruction of the Helsinki Tomography challenge data"""
    )
    ...
    parser.add_argument("--iters", type=int, default=70, help="Number of iterations")
    parser.add_argument(
        "--reg-param", type=float, default=7.0, help="Regularization parameter"
    )
    parser.add_argument(
    ...
    sinogram, volume_desc = load_htc2022data(file, dataset_name=dataset)
    sinogram = apply_BHC(sinogram)

    if elsa.cudaProjectorsEnabled():
        projector = elsa.JosephsMethodCUDA(volume_desc, sinogram.getDataDescriptor())
    else:
        projector = elsa.JosephsMethod(volume_desc, sinogram.getDataDescriptor())

    niters = args.iters
    reg = args.reg_param
    recon = reconstruction(projector, sinogram, niters=niters, reg=reg)
    ...
    if args.no_show:
        ...
        im = ax1.imshow(sinogram, cmap="gray")
        ax1.axis("equal")
        ax1.axis("off")
        ...
        ax2.axis("off")
        ax2.set_title(
            f"FISTA Reconstruction with {niters} iterations and regularization parameter {reg}"
        )

...
def reconstruction(A, b, niters=200, tau=1, lam=1):
    """Reconstruction with ADMM solving TV regularization"""
    grad = elsa.FiniteDifferences(A.getDomainDescriptor())
    proxg = elsa.ProximalL1(lam)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="ADMM-TV reconstruction of the Helsinki Tomography challenge data"
    )
    ...
    parser.add_argument(
        "--lam", type=float, default=10.0, help="Regularization parameter"
    )
    parser.add_argument(
        "--tau", type=float, default=0.05, help="Relaxation parameter for ADMM"
    )
    parser.add_argument(
    ...
    tau = args.tau
    lam = args.lam
    recon = reconstruction(A, b, niters=niters, tau=tau, lam=lam)
    ...
    if args.no_show:
        ...
        ax2.axis("off")
        ax2.set_title(
            f"ADMM reconstruction with {niters} iterations and params:\n{tau = },  lambda = {lam}"
        )

 ...
 * \]
 * using a proximal gradient descent (PGD) algorithm. This is also often referred to as
 * ISTA.
 *
 * PGD solves the unconstrained minimization problem of the form:
 * \[
 * \min_x f(x) + g(x)
 * \]
...
void example2d()
{
    ...
    // eigenvalue
    PGD<real_t> solver(projector, sinogram, ProximalBoxConstraint<real_t>{0});
    ...
    io::write(recon, "reco_pgd_ls-nonneg_tomo.pgm");
}
